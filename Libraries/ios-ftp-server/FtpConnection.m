...
- (id)initWithAsyncSocket:(AsyncSocket *)newSocket forServer:(id)myServer {
	if ((self = [super init]) != nil) {
		...
		currentUser = nil;
		TRACE(@"FC: Current Directory starting at %@",currentDir );
	}
	return self;
}
...
- (BOOL)openDataSocket:(int)portNumber {
	...
	switch (transferMode) {
		...
		case lprtftp: {					// FIXME wrong return message
			dataPort = portNumber;
			responseString = @"228 Entering Long Passive Mode 	(af, hal, h1, h2, h3,..., pal, p1, p2...)";
			[ dataSocket connectToHost:[self connectionAddress] onPort:portNumber error:&error ];
			dataConnection = [[ FtpDataConnection alloc ] initWithAsyncSocket:dataSocket forConnection:self withQueuedData:queuedData ];	
			break;
		}
...
// ----------------------------------------------------------------------------------------------------------
{
	...
	//	TRACE(@"New Port number is %i", portNumber );
...
// ----------------------------------------------------------------------------------------------------------
{
	...
	TRACE(@"FC:New Connection -- should be for the data port");
...
// ----------------------------------------------------------------------------------------------------------
{
	...
	{
		// Append data on
		TRACE(@"FC:Writing File to %@", currentFile );
		[ currentFileHandle writeData:dataConnection.receivedData ];
		
	}
	else
	{
		TRACE(@"Couldnt write data");
	}
...
- (void)didFinishReading {							// Called at the end of a data connection from the client we presume
	if (currentFile) {
		...
		TRACE(@"FC:Data Sent but not sure where its for ");
	}
...
// ----------------------------------------------------------------------------------------------------------
{
	...
	TRACE( @"doList currentDir(%@) changeRoot%d", lsDir, server.changeRoot );
	TRACE(@"Will list %@ ",lsDir);
	listText = [server createList:lsDir];						// Can list directory so do it
...
//	}
	...
	TRACE( @"doList sending this. %@", listText );
...
// ----------------------------------------------------------------------------------------------------------
{
	...
	TRACE(@"Will PWD %@ ",[self visibleCurrentDir]);
...
// ----------------------------------------------------------------------------------------------------------
{
	...
	TRACE(@"Got Send Port %d", clientPort );
...
// ----------------------------------------------------------------------------------------------------------
{
	...
	TRACE(@"DoCwd arguments is %@",arguments);
	...
	TRACE(@"currentDir is now %@",currentDir );	
...
{	
	...
	{
		TRACE(@"FC:setting connection state to clientSending");
		dataConnection.connectionState = clientSending;
	}
	else
	{
		TRACE(@"FC:Erorr  Cant set connection state to Client Sending : no Connection yet ");
	}
...
// ----------------------------------------------------------------------------------------------------------
{
	...
	else			// doesn't exist or not in basedir sandbox
	{
		...
		TRACE(@"FC:doRetr: file %@ doesnt' exist ", filePath);
		[sender sendMessage:cmdstr];
	}
...
// ----------------------------------------------------------------------------------------------------------
{
	...
	{				
		...
		{
			cmdStr = @"250 DELE command successful.";
			[ server didReceiveFileListChanged];				// Addition by Brendan copied in by Rich 16/12/08
		}
		else
		{
			cmdStr = @"550 DELE command unsuccessful.";				// FIXME put correct error code in
		}
	}
	else
	{
...
// ----------------------------------------------------------------------------------------------------------
{
	TRACE(@"current dir is %@",currentDir);
...
// ----------------------------------------------------------------------------------------------------------
{
	TRACE(@"CurrentDir is %@",[self visibleCurrentDir]);
...
// ----------------------------------------------------------------------------------------------------------
{
	...
	TRACE(@"File size is %qu ", [fileSize unsignedLongLongValue]);
...
- (NSString *)fileNameFromArgs:(NSArray *)arguments {
	...
	TRACE(@"%@", [arguments description]);

	if ([arguments count] > 1) {
		BOOL isList = [[arguments objectAtIndex:0] isEqualToString:@"LIST"];
		for (int n = 1; n < [arguments count]; n++) {
			// Start at arg2,
			// test to see if argument begins with a - as it might be just a parameter, not the name
			if (![[[arguments objectAtIndex:n] substringToIndex:1] isEqualToString:@"-"] || !isList) {
