...
- (BOOL)acceptOnAddress:(NSString *)hostaddr port:(UInt16)port error:(NSError **)errPtr
{
	...
	if (theSocket)
	{
		...
		//TRACE(@"theSocket4: %hu", [self localPort:theSocket]);
	}
	...
	if (theSocket6)
	{
		...
		//TRACE(@"theSocket6: %hu", [self localPort:theSocket6]);
	}
...
- (BOOL)createStreamsFromNative:(CFSocketNativeHandle)native error:(NSError **)errPtr
{
	...
	if (theReadStream == NULL || theWriteStream == NULL)
	{
		...
		TRACE (@"AsyncSocket %p couldn't create streams from accepted socket: %@", self, err);
...
- (BOOL)attachStreamsToRunLoop:(NSRunLoop *)runLoop error:(NSError **)errPtr
{
								...
								(CFStreamClientContext *)(&theContext) ))
	{
		...
		TRACE (@"AsyncSocket %p couldn't attach read stream to run-loop,", self);
		TRACE (@"Error: %@", err);
								 ...
								 (CFStreamClientContext *)(&theContext) ))
	{
		...
		TRACE (@"AsyncSocket %p couldn't attach write stream to run-loop,", self);
		TRACE (@"Error: %@", err);
...
- (BOOL)openStreamsAndReturnError:(NSError **)errPtr
{
	...
	{
		TRACE (@"AsyncSocket %p couldn't open read stream,", self);
		pass = NO;
	}
	
	if(pass && !CFWriteStreamOpen (theWriteStream))
	{
		TRACE (@"AsyncSocket %p couldn't open write stream,", self);
		pass = NO;
	}
...
- (void)doStreamOpen
{
	...
	if ([self areStreamsConnected] && !(theFlags & kDidCallConnectDeleg))
	{
		...
		{
			TRACE (@"AsyncSocket %p couldn't get socket from streams, %@. Disconnecting.", self, err);
			[self closeWithError:err];
			return;
		}
				  ...
				  withData:(const void *)pData
{
	...
	switch (type)
	{
		...
		case kCFSocketAcceptCallBack:
			...
			break;
		default:
			TRACE (@"AsyncSocket %p received unexpected CFSocketCallBackType %ld.", self, type);
			break;
	}
}
...
- (void)doCFReadStreamCallback:(CFStreamEventType)type forStream:(CFReadStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventErrorOccurred:
			...
			break;
		default:
			TRACE (@"AsyncSocket %p received unexpected CFReadStream callback, CFStreamEventType %ld.", self, type);
	}
}
...
- (void)doCFWriteStreamCallback:(CFStreamEventType)type forStream:(CFWriteStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
			TRACE (@"AsyncSocket %p received unexpected CFWriteStream callback, CFStreamEventType %ld.", self, type);
	}
}
