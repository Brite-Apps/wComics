...
// ----------------------------------------------------------------------------------------------------------
{
    if( self = [super init] ) {
		...
		if ( ! [ [ NSFileManager defaultManager ] fileExistsAtPath:plistPath ] )
			{
				TRACE(@"ftp_commands.plist missing");
			}
		...
		TRACE(@"Listening on %u", portNumber);
...
- (void)onSocket:(AsyncSocket *)sock didAcceptNewSocket:(AsyncSocket *)newSocket {
	...
	TRACE(@"FS:didAcceptNewSocket  port:%i", [sock localPort]);

	if ([sock localPort] == portNumber) {
		TRACE(@"Connection on Server Port");
	}
	else {
		...
		// and attach it
		TRACE(@"--ERROR %i, %i", [sock localPort],portNumber);
	}
}

- (void)onSocket:(AsyncSocket *)sock didConnectToHost:(NSString *)host port:(UInt16)port {
	TRACE(@"FtpServer:didConnectToHost  port:%i", [sock localPort]);
}
